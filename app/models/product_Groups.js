...
const MainSchemas	 = require(__path_schemas + 'product_Groups');
const SysPermissionModel = require(__path_models + 'sys_Permission');
const LogModel = require(__path_models + 'systemLog'); 
const systemConfig = require(__path_configs + 'system');
const functionBranch = 'Product_Groups';//use this parameter to check permissions: save, select, delete.... 
...
async function deleteById(Id, userLogin = null){
	...
	let sql = ` delete from Product_Groups 
		where (Id like @Id) `;
...
async function listItems(params, userLogin, options = null){
	...
	let sql = `  SELECT A.Id, A.Name, A.Ordering, A.Description, A.Is_Using, A.User_Id_Created, A.User_Name_Created, A.DateTime_Created, A.User_Id_Modified, A.User_Name_Modified, A.DateTime_Modified 
			, COUNT(*) OVER() as TotalRecord
			FROM Product_Groups A    ${where}
			ORDER BY ${sort}
			offset ${skip} rows
...
async function listItemForDropdown(){
	 let sql = ` SELECT Id, Name, Ordering, Description, Is_Using
			FROM Product_Groups  `;
...
async function exportData(item, userLogin){
	...
	let sql = ` SELECT A.Id, A.Name, A.Ordering, A.Description, A.Is_Using, A.User_Id_Created, A.User_Name_Created, A.DateTime_Created, A.User_Id_Modified, A.User_Name_Modified, A.DateTime_Modified 
			 FROM Product_Groups A  where (A.Id > 0 )`; 
		sql += where; 
	let result =  await dbConnection.executeQuerySchemaParam(MainSchemas.schemas, sql, dataParams );
	return result.recordset;
}
