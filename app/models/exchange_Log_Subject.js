...
const MainSchemas	 = require(__path_schemas + 'exchange_Log_Subject');
const SysPermissionModel = require(__path_models + 'sys_Permission');
const LogModel = require(__path_models + 'systemLog'); 
const systemConfig = require(__path_configs + 'system');
const functionBranch = 'Exchange_Log_Subject';//use this parameter to check permissions: save, select, delete.... 
...
async function deleteById(Id, userLogin = null){
	...
	let sql = ` delete from Exchange_Log_Subject 
		where (Id like @Id) `;
...
async function listItems(params, userLogin, options = null){
	...
	let sql = `  SELECT A.Id, A.Name, A.Ordering, A.Description, A.Status, A.User_Id_Created, A.User_Name_Created, A.DateTime_Created, A.User_Id_Modified, A.User_Name_Modified, A.DateTime_Modified 
			...
			FROM Exchange_Log_Subject A    ${where}
			ORDER BY ${sort}
			offset ${skip} rows
...
async function getById( Id, userLogin = null){
	...
	let sql = ` SELECT A.Id, A.Name, A.Ordering, A.Description, A.Status, 
		...
		FROM Exchange_Log_Subject as A
		where (A.Id = @Id) `; 
...
async function exportData(item, userLogin){
	...
	let sql = ` SELECT A.Id, A.Name, A.Ordering, A.Description, A.Status, A.User_Id_Created, A.User_Name_Created, A.DateTime_Created, A.User_Id_Modified, A.User_Name_Modified, A.DateTime_Modified 
			 FROM Exchange_Log_Subject A  where (A.Id > 0 )`; 
		sql += where; 
	let result =  await dbConnection.executeQuerySchemaParam(MainSchemas.schemas, sql, dataParams );
	return result.recordset;
}
