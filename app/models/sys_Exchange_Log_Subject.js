//**************************************************************************************************************************
//     Creation time: Saturday, 27 May 2023 10:35 AM
//     Creator: 
//**************************************************************************************************************************
var moment = require('moment');
const dbConnection	= require(__path_helpers + 'utils-mssql');
const MainSchemas	 = require(__path_schemas + 'sys_Exchange_Log_Subject');
const SysPermissionModel = require(__path_models + 'sys_Permission');
const LogModel = require(__path_models + 'systemLog'); 
const systemConfig = require(__path_configs + 'system');
const functionBranch = 'Sys_Exchange_Log_Subject';//use this parameter to check permissions: save, select, delete.... 
...
async function deleteById(Id, userLogin = null){
	...
	let sql = ` delete from Sys_Exchange_Log_Subject 
		where (Id like @Id) `;
...
async function listItems(params, userLogin, options = null){
	...
	let sql = `  SELECT A.Id, A.Name, A.Ordering, A.Description, A.Status, A.User_Id_Created, A.User_Name_Created, A.DateTime_Created, A.User_Id_Modified, A.User_Name_Modified, A.DateTime_Modified 
			, COUNT(*) OVER() as TotalRecord
			FROM Sys_Exchange_Log_Subject A    ${where}
			ORDER BY ${sort}
			offset ${skip} rows
...
async function listItemForDropdown(){
	 let sql = ` SELECT Id, Name, Ordering, Description, Status
			FROM Sys_Exchange_Log_Subject  `;
...
async function exportData(item, userLogin){
	...
	let sql = ` SELECT A.Id, A.Name, A.Ordering, A.Description, A.Status, A.User_Id_Created, A.User_Name_Created, A.DateTime_Created, A.User_Id_Modified, A.User_Name_Modified, A.DateTime_Modified 
			 FROM Sys_Exchange_Log_Subject A  where (1 = 1)`; 
		sql += where; 
	let result =  await dbConnection.executeQuerySchemaParam(MainSchemas.schemas, sql, dataParams );
	return result.recordset;
}
