...
const collection = "sys_Company";//use this parameter to check permissions: save, select, delete.... 
...
// // end getAll ================================================================================================= =================================================================================================
	...
	searchGet: async function(req, res, next){
		...
		let languageString 	= JSON.stringify(await publicFunction.getLanguageRequest(req, name_Cookie_Language));
		let item = {
			CompanyName : await ParamsHelpers.getParam(req.query, 'CompanyName', ''),
			Address : await ParamsHelpers.getParam(req.query, 'Address', ''),
			Email1 : await ParamsHelpers.getParam(req.query, 'Email1', ''),
			PhoneNumber1 : await ParamsHelpers.getParam(req.query, 'PhoneNumber1', ''),
			Notes : await ParamsHelpers.getParam(req.query, 'Notes', ''),
			RepresentativeName : await ParamsHelpers.getParam(req.query, 'RepresentativeName', ''),
			LinkFaceBook : await ParamsHelpers.getParam(req.query, 'LinkFaceBook', ''),
			BankAccountNumber1 : await ParamsHelpers.getParam(req.query, 'BankAccountNumber1', ''),
			BankName1 : await ParamsHelpers.getParam(req.query, 'BankName1', ''),
		};
		item = clean(item);
...
deleteById: async function(req, res, next) {
	try{
		...
		let item = Object.assign(req.body);
		item = clean(item);
		await MainModel.deleteById(item.CompanyId, req.user).then((result) => {
			res.status(200).json({
				'success': result
			});
		});
