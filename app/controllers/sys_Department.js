...
const collection = "sys_Department";//use this parameter to check permissions: save, select, delete.... 
...
const pageTitleIndex = 'Department List Management';
...
deleteById: async function(req, res, next) {
	try{
		...
		let item = Object.assign(req.body);
		item = clean(item);
		await MainModel.deleteById(item.DepartmentId, req.user).then((result) => {
			res.status(200).json({
				'success': result
			});
		});
