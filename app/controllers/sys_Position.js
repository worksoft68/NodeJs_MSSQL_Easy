...
const collection = "sys_Position";//use this parameter to check permissions: save, select, delete.... 
...
const pageTitleIndex = 'Position List Management';
...
// // end getAll ================================================================================================= =================================================================================================
	...
	searchGet: async function(req, res, next){
		...
		let languageString 	= JSON.stringify(await publicFunction.getLanguageRequest(req, name_Cookie_Language));
		let item = {
			PositionName : await ParamsHelpers.getParam(req.query, 'PositionName', ''),
		};
		item = clean(item);
...
deleteById: async function(req, res, next) {
	try{
		...
		let item = Object.assign(req.body);
		item = clean(item);
		await MainModel.deleteById(item.PositionId, req.user).then((result) => {
			res.status(200).json({
				'success': result
			});
		});
