...
const collection = "sys_FunctionForPermission";//use this parameter to check permissions: save, select, delete.... 
...
const pageTitleIndex = 'Function For Permission Management';
...
// // end getAll ================================================================================================= =================================================================================================
	...
	searchGet: async function(req, res, next){
		...
		let item = {
			...
			ModuleSystem : await ParamsHelpers.getParam(req.query, 'ModuleSystem', ''),
		};
		item = clean(item);
...
deleteById: async function(req, res, next) {
	try{
		...
		let item = Object.assign(req.body);
		item = clean(item);
		await MainModel.deleteById(item.FunctionName, req.user).then((result) => {
			res.status(200).json({
				'success': result
			});
		});
