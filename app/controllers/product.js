...
const collection = "product";//use this parameter to check permissions: save, select, delete.... 
const MainModel = require(__path_models + collection);
const MainValidate  = require(__path_validates + collection);
const Product_GroupsModel = require(__path_models + 'product_Groups');
...
module.exports = {
	...
	indexString: async function(req, res, next) {
		...
		const catalogue = [];
		product_groups = await Product_GroupsModel.listItemForDropdown();
		catalogue.push(product_groups);
