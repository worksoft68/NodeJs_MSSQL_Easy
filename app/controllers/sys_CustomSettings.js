...
const uploadFile = FileHelpers.upload('Image', collection);// Image: control file name in HTML. sys_CustomSettings is folder in folder public 
const uploadFolder = 'public/uploads/' + collection + '/';
const pageTitleIndex = 'Customsettings List Management';
...
// // end getAll ================================================================================================= =================================================================================================
	...
	searchGet: async function(req, res, next){
		...
		let item = {
			...
			Value : await ParamsHelpers.getParam(req.query, 'Value', ''),
			DefaultValue : await ParamsHelpers.getParam(req.query, 'DefaultValue', ''),
...
// end searchGet =================================================================================================
	...
	save: async function(req, res, next) {
		...
		uploadFile(req, res, async (errUpload) => {
			...
			let item = Object.assign(req.body);
			item = clean(item);
			if ((item.UseCKEditor == 'on') || (item.UseCKEditor == 'yes') || (item.UseCKEditor == 'true') || (item.UseCKEditor == '1'))
				item.UseCKEditor = true;
			else item.UseCKEditor = false;
			if ((item.Status == 'on') || (item.Status == 'yes') || (item.Status == 'true') || (item.Status == '1'))
				item.Status = true;
			else item.Status = false;
			if ((item.IsSystem == 'on') || (item.IsSystem == 'yes') || (item.IsSystem == 'true') || (item.IsSystem == '1'))
				item.IsSystem = true;
			else item.IsSystem = false;
			let taskCurrent = (typeof item !== "undefined" && item.KeySettingsHidden != "") ? "edit" : "add";
...
deleteById: async function(req, res, next) {
	try{
		...
		let item = Object.assign(req.body);
		item = clean(item);
		await MainModel.deleteById(item.KeySettings, req.user).then((result) => {
			res.status(200).json({
				'success': result
			});
		});
