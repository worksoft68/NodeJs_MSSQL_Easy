...
const collection = "sys_User";//use this parameter to check permissions: save, select, delete.... 
...
const Sys_DepartmentModel = require(__path_models + 'sys_Department');
const Sys_PositionModel = require(__path_models + 'sys_Position');
const Sys_RoleModel = require(__path_models + 'sys_Role');
const Sys_User_RoleModel = require(__path_models + 'sys_User_Role');
...
const pageTitleIndex = 'Sys_User Management';
...
module.exports = {
	...
	indexString: async function(req, res, next) {
		...
		sys_department = await Sys_DepartmentModel.listItemForDropdown();
		catalogue.push(sys_department);
		sys_position = await Sys_PositionModel.listItemForDropdown();
		catalogue.push(sys_position);

		let sys_role = await Sys_RoleModel.listItemForDropdown();
...
// // end getAll ================================================================================================= =================================================================================================
	...
	searchGet: async function(req, res, next){
		...
		let catalogueString = JSON.stringify(catalogue);
		let item = {
			EmployeeCode : await ParamsHelpers.getParam(req.query, 'EmployeeCode', ''),
...
updateProfile: async function (req, res, next) {
	...
	const catalogue = [];
	let sys_department = await Sys_DepartmentModel.listItemForDropdown();
	catalogue.push(sys_department);		
	let catalogueString = JSON.stringify(catalogue);
	...
	uploadFile(req, res, async (errUpload) => {
		...
		if (errors != false) {
			...
			item["UsersId"] = userLogin.UsersId;
			await MainModel.saveUpdateProfile(item, userLogin).then(async(result) => {
				let user = await MainModel.getByIdCheckLogin(userLogin.UsersId);
...
deleteById: async function(req, res, next) {
	try{
		...
		let item = Object.assign(req.body);
		item = clean(item);
		await MainModel.deleteById(item.UsersId, req.user).then((result) => {
			res.status(200).json({
				'success': result
			});
		});
